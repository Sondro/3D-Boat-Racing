...
class InputManager 
{
	...
	public var strengthLeft: Array<Bool>;
	private var strength: Array<Float>;
	...
	public inline function getStrength(ID: Int, left: Bool): Float {
		if (left && !strengthLeft[ID])
			return 0.0;
		if (!left && strengthLeft[ID])
			return 0.0;
		
		var result = strength[ID];
		strength[ID] = 0.0;
		return result;
	}
	...
	private inline function onPush(ID: Int, left: Bool) {
		if (rightSide[ID] && !left || leftSide[ID] && left) {
			strength[ID] = getCharge(ID);
			leftSide[ID] = false;
			rightSide[ID] = false;
			strengthLeft[ID] = left;
